...
import { X, CheckCircle, AlertCircle, Loader2, FileText, Brain, Download } from 'lucide-react';
...
export default function CVGenerationModal({ 
  ...
  const simulateAIStreaming = (text: string) => {
    ...
    const interval = setInterval(() => {
      if (currentIndex < words.length) {
        setAiResponse(prev => prev + (currentIndex === 0 ? '' : ' ') + words[currentIndex]);
        currentIndex++;
      } else {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      updateStepStatus('validation', 'completed', 'Données validées avec succès');
      ...
      //     'Content-Type': 'application/json',
      //   },
      //   body: JSON.stringify({ cvData: aiResult.improvedCV }),
      // });
      ...
      // const filename = pdfResponse.headers.get('Content-Disposition')?.match(/filename="(.+)"/)?.[1] || 'cv-improved.pdf';
      ...
      if (currentStep) {
        updateStepStatus(currentStep.id, 'error', error instanceof Error ? error.message : 'Erreur inconnue');
      }
    } finally {
  ...
  return (
    <div data-testid="cv-generation-modal" className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div data-testid="cv-generation-modal-header" className="flex items-center justify-between p-6 border-b">
          <div>
        ...
        <div className="p-6">
          ...
          <div className="space-y-4 mb-6">
            {steps.map((step, index) => (
              <div key={step.id} data-testid={`cv-generation-step-${step.id}`} className="flex items-start space-x-3">
          ...
          {/* Current AI Step */}
          {currentAiStep && (
            <div data-testid="cv-generation-current-ai-step" className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
          ...
          {/* AI Response Stream */}
          {aiResponse && (
            <div data-testid="cv-generation-ai-response" className="bg-gray-50 border rounded-lg p-4 mb-4">
              ...
              <div className="text-sm text-gray-600 leading-relaxed">
                {aiResponse}
                {isGenerating && steps.find(s => s.id === 'ai-improvement')?.status === 'running' && (
          ...
          {/* Error Display */}
          {error && (
            <div data-testid="cv-generation-error" className="bg-red-50 border border-red-200 rounded-lg p-4">
              <div className="flex items-center space-x-2">
        ...
        {/* Footer */}
        <div data-testid="cv-generation-modal-footer" className="bg-gray-50 px-6 py-4 border-t">
          <div className="flex items-center justify-between">
            <div data-testid="cv-generation-status" className="text-sm text-gray-600">
              {isGenerating ? (
                <>
